...
let DetailsPage = class DetailsPage {
    constructor(navCtrl, statusBar, formBuilder, router, iab, route, ref, api) {
        ...
        this.route.queryParams.subscribe((params) => {
            this.api.propertyUserDetail(params.pid).subscribe((data) => {
    ...
    SwitchLanguage(LanguageName) {
        let i = 0;
        let languagesLength = this.languageArray.length;
        ...
        setTimeout(() => {
            this.RecomandationPropertyDataGet(this.property_content_id, this.propertyDetails.property_type_id, this.propertyDetails.location);
        }, 2000);
    }
    RecomandationPropertyDataGet(property_content_id, property_type_id, location) {
        var body = {
        ...
        this.loading = true;
        this.api.RecomandationGet(body).subscribe((data) => {
            if (data.status == 1) {
    ...
    send() {
        ...
        if (this.detail_form.valid) {
            ...
            this.loading = false;
            this.api.Negotitation(body).subscribe((data) => {
